...
class finite_automata:
    ...
    # Function used to display a grid made of the FA
    ...
    def display(self):
        ...
        for state in self.dict_transitions.keys():
            # Get the initial/final arrow line if needed
            line = "|"
            if state in self.list_initial_states and state in self.list_final_states:
                line += "<-->|"
            elif state in self.list_initial_states:
            ...
            # Get the state line with padding
            diff_word = padding[1] - len(state)
            left_padding = diff_word // 2
            right_padding = diff_word - left_padding
            line += " " * (left_padding + 1) + state + " " * (right_padding + 1) + "|"

            # Get the transition line with padding
            for i, symbol in enumerate(self.list_symbols):
                transition_line = ""
